    ...
    public class DataAccessLayer
    {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (users.Type == "User")
                    {
                        ...
                        order.CreatedOn = Convert.ToDateTime(dt.Rows[i]["CreatedOn"]);
        ...
        public Response addUpdateMedicine(Medicines medicines, SqlConnection connection)
        {
            ...
            if (medicines.Type != "Get" && medicines.Type != "GetByID")
            {
                ...
                {
                    response.StatusCode = 200;
                    response.StatusMessage = medicines.Type switch
                    {
                        "Add" => "Medicine inserted successfully",
                ...
                else
                {
                    ...
                    response.StatusMessage = medicines.Type switch
                    {
                        "Add" => "Medicine did not save. Try again.",
                        "Update" => "Medicine did not update. Try again.",
                        "Delete" => "Medicine did not delete. Try again.",
                        _ => "Operation failed. Try again."
                    };
                }
            }
            else
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        Medicines medicine = new Medicines
                        {
                            ...
                            ExpDate = Convert.ToString(dt.Rows[i]["ExpDate"]),
